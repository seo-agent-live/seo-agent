...
export default function SiteAuditPage() {
  ...
  return (
    <div style={{ color: '#fff', fontFamily: 'Inter, sans-serif', maxWidth: '900px' }}>
      <h1 style={{ fontSize: '28px', fontWeight: 700, marginBottom: '8px', background: 'linear-gradient(135deg, #fff, #94a3b8)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Site Audit</h1>
      <p style={{ color: '#64748b', marginBottom: '32px', fontSize: '15px' }}>Audit any page or site for SEO issues, meta tags, and performance.</p>
      <div style={{ display: 'flex', gap: '8px', marginBottom: '24px' }}>
        {[{ id: 'audit', label: 'Run Audit' }, { id: 'history', label: 'Audit History' }].map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ padding: '8px 18px', borderRadius: '8px', border: '1px solid', borderColor: activeTab === tab.id ? '#6366f1' : 'rgba(255,255,255,0.1)', background: activeTab === tab.id ? 'rgba(99,102,241,0.2)' : 'rgba(255,255,255,0.03)', color: activeTab === tab.id ? '#a5b4fc' : '#64748b', fontSize: '13px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>{tab.label}</button>
        ))}
      </div>
      {activeTab === 'audit' && (
        <div>
          <div style={{ background: 'linear-gradient(135deg, rgba(99,102,241,0.08), rgba(15,23,42,0.9))', border: '1px solid rgba(99,102,241,0.15)', borderRadius: '16px', padding: '28px', marginBottom: '24px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              <input value={url} onChange={e => setUrl(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAudit()} placeholder="https://yoursite.com" style={{ flex: 1, padding: '12px 16px', backgroundColor: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '10px', color: '#fff', fontSize: '14px', outline: 'none', fontFamily: 'inherit' }} />
              <button onClick={handleAudit} disabled={loading || !url.trim()} style={{ padding: '12px 24px', background: loading ? 'rgba(99,102,241,0.4)' : 'linear-gradient(135deg,#6366f1,#8b5cf6)', border: 'none', borderRadius: '10px', color: '#fff', fontSize: '14px', fontWeight: 600, cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }}>{loading ? 'Auditing...' : 'Run Audit'}</button>
            </div>
          </div>
          ...
          {loading && <div style={{ textAlign: 'center', padding: '60px', color: '#64748b' }}><div style={{ fontSize: '32px', marginBottom: '12px' }}>Ì¥ç</div><p>Running full site audit...</p></div>}
          {audit && !loading && (
            <div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '24px' }}>
                {[{ label: 'Overall Score', value: audit.overallScore, suffix: '/100' }, { label: 'Meta Tags', value: audit.metaScore, suffix: '/100' }, { label: 'Performance', value: audit.performanceScore, suffix: '/100' }, { label: 'Issues Found', value: audit.issues?.length || 0, suffix: '' }].map((s, i) => (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div style={{ padding: '20px', background: 'rgba(239,68,68,0.05)', border: '1px solid rgba(239,68,68,0.15)', borderRadius: '14px' }}>
                  <h3 style={{ fontSize: '15px', fontWeight: 600, color: '#ef4444', marginBottom: '12px' }}>Issues</h3>
                  {audit.issues?.map((issue, i) => <div key={i} style={{ fontSize: '13px', color: '#94a3b8', padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>‚Ä¢ {issue}</div>)}
                </div>
                <div style={{ padding: '20px', background: 'rgba(16,185,129,0.05)', border: '1px solid rgba(16,185,129,0.15)', borderRadius: '14px' }}>
                  ...
                  {audit.recommendations?.map((rec, i) => <div key={i} style={{ fontSize: '13px', color: '#94a3b8', padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>‚Ä¢ {rec}</div>)}
                </div>
              </div>
            </div>
          )}
      ...
      {activeTab === 'history' && (
        <div>
          ...
          {history.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '60px', color: '#64748b' }}><div style={{ fontSize: '32px', marginBottom: '12px' }}>Ì≥ã</div><p>No audits yet.</p></div>
          ) : (
