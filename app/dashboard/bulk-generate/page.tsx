  ...
  return (
    <div style={{ color: '#fff', fontFamily: 'Inter, sans-serif', maxWidth: '900px' }}>
      ...
      <p style={{ color: '#64748b', marginBottom: '32px', fontSize: '15px' }}>
        Generate multiple SEO articles at once. Enter one keyword per line.
      </p>
      ...
      <div style={{ background: 'linear-gradient(135deg, rgba(99,102,241,0.08), rgba(15,23,42,0.9))', border: '1px solid rgba(99,102,241,0.15)', borderRadius: '16px', padding: '28px', marginBottom: '24px' }}>
        <div style={{ marginBottom: '20px' }}>
          <label style={{ color: '#94a3b8', fontSize: '13px', display: 'block', marginBottom: '8px', fontWeight: 600 }}>
            Keywords (one per line) — {keywordList.length} keyword{keywordList.length !== 1 ? 's' : ''} entered
          </label>
          <textarea
            value={keywords}
            ...
            rows={6}
            style={{ width: '100%', padding: '12px 16px', backgroundColor: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '10px', color: '#fff', fontSize: '14px', outline: 'none', fontFamily: 'Inter, sans-serif', resize: 'vertical', boxSizing: 'border-box' }}
          />
        </div>
        ...
        {loading && (
          <div style={{ marginTop: '16px' }}>
            ...
            <p style={{ fontSize: '12px', color: '#64748b', marginTop: '8px', textAlign: 'center' }}>
              {results.length} of {keywordList.length} articles generated
            </p>
          </div>
        )}
      </div>
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            <h2 style={{ fontSize: '18px', fontWeight: 600, color: '#e2e8f0', margin: 0 }}>
              Generated Articles ({results.filter(r => r.status === 'done').length}/{keywordList.length})
            </h2>
            {!loading && results.some(r => r.status === 'done') && (
              <button onClick={handleDownloadAll} style={{ padding: '8px 16px', background: 'rgba(99,102,241,0.2)', border: '1px solid rgba(99,102,241,0.4)', borderRadius: '8px', color: '#a5b4fc', fontSize: '13px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
                ⬇ Download All
              </button>
            )}
          </div>
              ...
              <div key={i} style={{ padding: '20px', background: 'rgba(255,255,255,0.03)', border: '1px solid', borderColor: r.status === 'done' ? 'rgba(99,102,241,0.2)' : 'rgba(239,68,68,0.2)', borderRadius: '14px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: r.status === 'done' ? '12px' : 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <span style={{ fontSize: '18px' }}>{r.status === 'done' ? '✅' : '❌'}</span>
                    <span style={{ fontSize: '15px', fontWeight: 600, color: '#e2e8f0' }}>{r.keyword}</span>
                  </div>
                  {r.status === 'done' && (
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                      <span style={{ fontSize: '12px', color: '#64748b' }}>{r.wordCount} words · {r.readTime}</span>
                      <button onClick={() => handleCopy(r.content)} style={{ padding: '6px 12px', background: 'rgba(99,102,241,0.2)', border: '1px solid rgba(99,102,241,0.3)', borderRadius: '6px', color: '#a5b4fc', fontSize: '12px', cursor: 'pointer', fontFamily: 'inherit' }}>
                        Copy
                      </button>
                    </div>
                  )}
                ...
                {r.status === 'done' && (
                  <details>
                    <summary style={{ fontSize: '13px', color: '#6366f1', cursor: 'pointer', marginBottom: '8px' }}>Preview article</summary>
                    <pre style={{ whiteSpace: 'pre-wrap', fontFamily: 'Inter, sans-serif', fontSize: '13px', color: '#94a3b8', margin: 0, lineHeight: '1.7', maxHeight: '300px', overflow: 'auto' }}>
                      {r.content}
                    </pre>
                  </details>
                )}
                {r.status === 'error' && (
                  <p style={{ fontSize: '13px', color: '#ef4444', margin: 0 }}>{r.error}</p>
                )}
              </div>
            ))}
          </div>
